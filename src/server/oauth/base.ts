import { z } from 'zod'
import { Cookies } from '@/server/data/session'
import { OAuthProvider } from '@/server/db/schema/users'
import { BASE_URL } from '@/lib/constants'
import { createGoogleOAuthClient } from './google'
import { generateRandomHex, hashString } from '@/lib/utils'
...
export class OAuthClient<T> {
  ...
  private get redirectUrl() {
    return new URL(this.provider, `${BASE_URL}/api/auth/callback/`)
  }

  async createAuthUrl(cookies: Pick<Cookies, 'set'>) {
    ...
    url.searchParams.set('code_challenge', await hashString(codeVerifier))
    return url.toString()
  }
...
function createState(cookies: Pick<Cookies, 'set'>) {
  const state = generateRandomHex(64).normalize()
...
function createCodeVerifier(cookies: Pick<Cookies, 'set'>) {
  const codeVerifier = generateRandomHex(64).normalize()
